function _jsonURL(){return(
"https://data.cityofnewyork.us/resource/25th-nujf.json?$where=rnk<6"
)}

function _urlData(d3,jsonURL){return(
d3.json(jsonURL)
)}

function _rearrange(urlData){return(
urlData.reduce((col, item, idx) => {
    const name = item.nm.toUpperCase();
    if(!col[item.brth_yr]) {
      col[item.brth_yr] = {};
      col[item.brth_yr][name] = Number(item.cnt)
    } else if (!col[item.brth_yr][name]){
      col[item.brth_yr][name] = Number(item.cnt)
    } else {
      col[item.brth_yr][name] += Number(item.cnt)
    }
...
function _newData(rearrange){return(
Object.keys(rearrange).map((item, idx) => {
    return Object.keys(rearrange[item]).map((name) => {
      return { date: Number(item), name: name, value: rearrange[item][name] }
    })
  })
)}

function _data(newData){return(
newData.reduce((col, item, idx) => {
    col = [...col, ...item];
    return col
  }, [])
)}

function _setupDate(d3){return(
d3.utcFormat("%Y")
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("jsonURL")).define("jsonURL", _jsonURL);
  main.variable(observer("urlData")).define("urlData", ["d3","jsonURL"], _urlData);
  main.variable(observer("rearrange")).define("rearrange", ["urlData"], _rearrange);
  main.variable(observer("newData")).define("newData", ["rearrange"], _newData);
  main.variable(observer("data")).define("data", ["newData"], _data);
  main.variable(observer("setupDate")).define("setupDate", ["d3"], _setupDate);
